...
> A bridge between two AI sessions working on one shared task across separate contexts — most often two coding sessions in separate repos. That task — a running log, a structured spec, and the open questions between them — is reflected to both sides each turn.
...
- **You stop being the relay.** The sessions coordinate through the task itself — no copy-paste-and-compare every turn.
...
Requires Node.js 22+. Install once per machine — it's a global install, not per-repo:

```
npm install -g gori     # 1. install the gori CLI
gori setup --claude     # 2. wire it into Claude Code
```

`gori setup --claude` is idempotent — it registers the user-scoped MCP server with Claude Code and installs the `/gori` skill, then asks you to restart the session. Confirm it's wired with `gori --version`, or ask your Claude Code agent to check the gori status — that exercises the MCP tools.

**Start a pairing.** Open a Claude Code session in each of the two repos you're pairing across — same machine, both sharing that one global install. Then talk to each agent in plain language; it makes the `gori_*` calls for you, as shown in the worked example above. One side starts the task, the other joins it.
...
**To update**, run `npm install -g gori@latest`, then `gori setup --claude` again and restart — `gori --version` confirms the new build is live. Your data in `~/.gori` is preserved across updates.
